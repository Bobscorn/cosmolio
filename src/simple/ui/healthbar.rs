...
use crate::simple::gameplay::actor::{ActorContext, Stat};
...
pub fn c_update_healthbars(
    ...
    for (mut sprite, health_bar) in &mut healthbars
    {
        ...
        let Some(max_health) = parent_actor.get_stat(&Stat::MaxHealth) else { warn!("health bar on entity with no max health!"); continue; };
        let Some(cur_health) = parent_actor.get_stat(&Stat::Health) else { warn!("Health Bar on entity with no health stat!"); continue; };
        let percent_health;
        if max_health <= 0.0
        {
            percent_health = 0.0;
