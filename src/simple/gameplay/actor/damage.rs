...
use crate::simple::gameplay::Position;
...
/// Useful for storing where a piece of damage came from
#[derive(PartialEq, Clone, Copy, Debug, Serialize, Deserialize, Reflect)]
pub enum DamageSource
{
    Actor(Entity), // Don't assume this entity stil exists
    Other,
}

/// Event to send to trigger damage events in an easy way
...
/// Describes the knockback that an 'object' should do related to it's position and the position of something else (usually what it collides with).
...
/// Component used to store the info about the damage an 'object' does.
/// Generally requires an ActorChild component and some method of triggering the damage (typically collision with a rapier collider) to be useful.
...
fn do_dmg(
    ...
    actor_context.last_damage_source = Some(super::DamageSource::Actor(instigator));
}
