...
use crate::simple::{
    consts::CLIENT_STR,
    gameplay::actor::{ActorContext, Stat},
    player::LocalPlayer,
};
...
pub fn s_kill_zero_healths(
    ...
    {
        let Some(health) = actor.get_stat(&Stat::Health) else { continue; };
        if health > 0.0
        {
            continue;
        }
...
pub fn cs_update_trans_system(mut things: Query<(&Position, &mut Transform), Without<bevy_rapier2d::prelude::RigidBody>>)
{
...
pub fn cs_update_orientation_system(mut objects: Query<(&Orientation, &mut Transform)>)
{
