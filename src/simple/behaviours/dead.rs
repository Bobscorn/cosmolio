...
pub fn s_destroy_dead_things(
    ...
    for (entity, child) in &dead_children
    {
        ...
        {
            apply_on_ability_end_effects(child.ability_type, &mut ActorEffectContext{ actor_entity: parent_ent, actor: &mut actor_context, commands: &mut commands, location: &mut position });
        }
    ...
    {
        apply_on_death_effects(&mut ActorEffectContext{ actor_entity: entity, actor: &mut actor, commands: &mut commands, location: &mut position });
        commands.entity(entity).despawn_recursive();
    }
